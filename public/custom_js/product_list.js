    ...
    product_list_tbl.then(function(data){
        $('#product_list_table').DataTable( {
            ...
            columns: [
                ...
                { title: "Category Name" },
                { title: "Product Price (English)" },
                { title: "Product Price (Arabic)" },
            ...
            "columnDefs": [{
                ...
                targets: 2,
                render : function (data, type, row) {
                    return '<img style="width: 80px;" src="'+APP_URL+'/cms_media/productimage/'+data+'" alt="image"/>'    
                }
            }]
        });
    });
        ...
        $.ajax({
            ...
            .done(function(response) {
                if(response.status_code == 200){
                        ...
                        $(".en_product_title").val(response.data.en_product_title);
                        $(".ar_product_title").val(response.data.ar_product_title);
                        $(".category_id").val(response.data.category_id).attr('selected','selected'); 
                        $(".en_product_description").val(response.data.en_product_description);
                        $(".ar_product_description").val(response.data.ar_product_description);
                        $(".en_product_price").val(response.data.en_product_price);
                        $(".ar_product_price").val(response.data.ar_product_price);
                        $('#img_product').attr('src', APP_URL + '/cms_media/productimage/'+response.data.product_image_url);
                        $('#product_image_url').val(response.data.product_image_url);
    ...
    $(document).on("click",".upload_imagetest",function(){
        ...
        if(total_file > 0){
            for(var i=0;i<total_file;i++)
            {
                ...
                $('#product_image_url').val(event.target.files[i].name); 
            }    
        } 
        });
    });
...
function resetIncomingProduct(){
    ...
    $(".en_product_title").val();
    $(".ar_product_title").val();
    $(".en_product_description").val();
    $(".ar_product_description").val();
    $(".en_product_price").val();
    $(".ar_product_price").val();
