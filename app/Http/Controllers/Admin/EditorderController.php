...
{   
    ...
    public function getallOrderlist($editorderid){
        $data = [];
        $getallOrderlist = Orderitems::leftjoin('orders', 'orders.id', '=', 'orderitems.item_order_id')->leftjoin('product', 'product.id', '=', 'orderitems.product_id')->where('item_order_id', Helper::decrypt($editorderid))->select('orderitems.*', 'product.en_product_title', 'product.en_product_price')->get()->toArray();
        
        if(count($getallOrderlist) > 0){
            foreach ($getallOrderlist as $allOrderlist) {
                $data[] = [$allOrderlist['id'], $allOrderlist['en_product_title'], $allOrderlist['en_product_price'], $allOrderlist['orderitems_qty']."-".$allOrderlist['id'], $allOrderlist['orderitems_qty']*$allOrderlist['en_product_price']."-".$allOrderlist['id'], Helper::encrypt($allOrderlist['id'])];
            }
        }
    ...
    public function getuserorderlist($editorderid){
        ...
        $getOrderuseritems = Orderitems::leftjoin('product', 'product.id', '=', 'orderitems.product_id')->where('orderitems.item_order_id', Helper::decrypt($editorderid))->select('orderitems.*', 'product.en_product_title', 'product.en_product_price')->get();
