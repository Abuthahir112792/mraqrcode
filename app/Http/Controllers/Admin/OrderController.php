...
{   
    ...
    public function incomingOrders(){
        $data = [];
        $getOrders = Orders::leftjoin('users', 'users.id', '=', 'orders.user_id')->where('order_status', '')->orwhere('order_status', 'Pending')->orWhereNull('order_status')->select('orders.*', 'users.mobile_number', 'users.address')->get()->toArray();
        
        if(count($getOrders) > 0){
            foreach ($getOrders as $order) {
                $date_time = date('d-m-Y H:i', strtotime($order['created_at']));
                $data[] = [$order['id'],$date_time, $order['order_id'], $order['user_name'], $order['order_description'], $order['order_price'], $order['order_status']."-".$order['id'], $order['mobile_number'], $order['address'], $order['self_pickup'], Helper::encrypt($order['id'])];
            }
        }
    ...
    public function outForDeliveryOrders(){
        $data = [];
        $getOrders = Orders::leftjoin('users', 'users.id', '=', 'orders.user_id')->where('order_status', 'Accepted')->where('self_pickup', 'False')->select('orders.*', 'users.mobile_number', 'users.address')->get()->toArray();
        
        if(count($getOrders) > 0){
            foreach ($getOrders as $order) {
                $date_time = date('d-m-Y H:i', strtotime($order['created_at']));
                $data[] = [$order['id'],$date_time, $order['order_id'], $order['user_name'], $order['order_description'], $order['order_price'], $order['order_status']."-".$order['id'], $order['mobile_number'], $order['address'], Helper::encrypt($order['id'])];
            }
        }
    ...
    public function selfPickupOrders(){
        $data = [];
        $getOrders = Orders::leftjoin('users', 'users.id', '=', 'orders.user_id')->where('order_status', '=' ,'Accepted')->where('self_pickup', '=' ,'True')->select('orders.*', 'users.mobile_number', 'users.address')->get()->toArray();
    ...
    public function getOrderitemDetails($id){
        ...
        $getOrderitems = Orderitems::leftjoin('product', 'product.id', '=', 'orderitems.product_id')->where('orderitems.item_order_id', Helper::decrypt($id))->select('orderitems.*', 'product.en_product_title', 'product.en_product_price')->get()->toArray();
        
        if(count($getOrderitems) > 0){
            foreach ($getOrderitems as $orderitem) {
                $data[] = [$orderitem['id'], $orderitem['en_product_title'], $orderitem['en_product_price'], $orderitem['orderitems_qty'], $orderitem['orderitems_qty']*$orderitem['en_product_price']];
            }
        }
    ...
    public function neworderAllow($status){
     ...
     public function neworderChecking(){
       ...
       // $to3 = date('Y-m-d H:i',strtotime('-3 minutes',strtotime($from)));
        ...
        $neworderstatus123 = Orderstatus::select('status')->first();
