...
class Helper
{
    ...
    public static function getuserAllDetails($id){
    ...
    public static function getMenuAllDetails($id){
        $data = Menu::where('id', Helper::decrypt($id))->first();
        if($data){
            return response()->json(['status' => 'Success!','status_code' => 200,'data' => $data]);
        }
        return response()->json(['status' => 'Error!','status_code' => 100]);
    ...
    public static function getCategoryAllDetails($id){
        ...
        $categorymenuoutput = '<option value="">Select</option>';
        foreach($categorymenu as $menu)
        {
        $categorymenuoutput .= '<option value="'.$menu->id.'">'.$menu->en_menu_title."-".$menu->ar_menu_title.'</option>';
        }
        if($data){
            return response()->json(['status' => 'Success!','status_code' => 200,'data' => $data,'categorymenuoutput' => $categorymenuoutput]);
        }
        return response()->json(['status' => 'Error!','status_code' => 100]);
    }
    ...
    public static function getProductAllDetails($id){
        ...
        {
        $productoutput .= '<option value="'.$category->id.'">'.$category->en_category_title."-".$category->ar_category_title.'</option>';
        }
        if($data){
